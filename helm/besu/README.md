...
* All nodes are validators, so each node needs a public and private key. They're both created with `scripts/generate-keys.sh`, but only the private keys are mounted to each pod as a Kubernetes secret. The public keys are used to peer other nodes.
...
* Key management could be improved with external secret management like Vault instead of leaning on Kubernetes secrets.